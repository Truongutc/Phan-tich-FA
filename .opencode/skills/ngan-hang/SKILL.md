...
# Công thức tính COE (bổ sung 2.0% phần bù rủi ro đặc thù quốc gia/ngân hàng)
COE = Rf + Beta * ERP + 2.0% (Specific Risk Premium)

# Công thức tính Continuing Value bảo thủ (áp dụng Phương án B: chiết khấu tiếp 50%)
CV = (RI(3) × (1+g) / (COE - g)) × 0.5
```
...
- **Vay/Tiền gửi (LDR)**: `=Loans / (Deposits + Bonds)` (Bắt buộc cộng thêm Giấy tờ có giá vào mẫu số).
- **Trục hoành biểu đồ chất lượng tài sản (NPL, LLR, CoC)**: Bắt buộc ghép điểm dữ liệu của **Quý gần nhất** thực tế (`Q_gần_nhất`, ví dụ `Q1/26`) trực tiếp vào sau chuỗi các năm lịch sử (ví dụ: `2021, 2022, 2023, 2024, 2025, Q1/26`) thay vì để dự báo ảo, giúp đánh giá chính xác hiện trạng chất lượng tài sản thời điểm hiện tại.
- **Biểu đồ kết quả kinh doanh (NII, TOI, NPAT...)**: Không ghép năm và quý chung trục vì quy mô lệch pha (thời kỳ), chỉ so sánh YoY/QoQ riêng biệt.
...
## 19. TÍNH TOÁN CHỈ SỐ THEO TỪNG QUÝ (QUARTERLY RATIOS) & EARNING RELEASE
 
 Để hiển thị chính xác diễn biến kết quả kinh doanh quý gần nhất và YTD, hệ thống bắt buộc phải tính chỉ số theo quý trực tiếp từ dữ liệu BCTC quý lịch sử:
 ...
 6. **Bảng so sánh Earning Release**: Bắt buộc thiết lập bảng so sánh KQKD của quý mới nhất đối chiếu với quý liền trước (QoQ %) và quý cùng kỳ năm trước (YoY %) cho các chỉ tiêu chính (NII, TOI, OPEX, PPOP, Dự phòng, LNTT, LNST) để xuất bản trực quan trong PDF.
 7. Đóng gói mảng chỉ số quý này vào trường `ratios_quarterly` và `analysis_comments` trong file JSON Summary để Dashboard đọc tĩnh.
 
 ---

## 20. CÁCH XÁC ĐỊNH SỐ CỔ PHIẾU CHUẨN XÁC (SHARES CALCULATION)

- Tránh việc bị API Cloudflare chặn khi fetch thông tin ngoài `/company/details`.
- Cách tính Shares an toàn và chính xác tuyệt đối: Đọc Vốn điều lệ (`bsa80` hoặc `bsb118` trong Balance Sheet năm gần nhất) và chia cho mệnh giá **10,000 VND**.
- Công thức: `shares = charter_capital / 10000`.

---

## 21. QUY TẮC TỰ ĐỘNG PHÂN LOẠI NGÀNH THÔNG MINH

- Khi chạy pipeline phân tích, hệ thống tự động kiểm tra cấu trúc tài khoản Balance Sheet của doanh nghiệp.
- Nếu Balance Sheet chứa các mã tài khoản tiền gửi `bsb113`, vay `bsb103` hoặc giấy tờ có giá `bsb116`, hệ thống **bắt buộc tự nhận diện là ngân hàng** và chạy `template_banking.py`.

---

## 22. YÊU CẦU GIẢI TRÌNH GIẢ ĐỊNH DỰ BÁO LỢI NHUẬN (ASSUMPTION TRANSPARENCY)

- Báo cáo PDF bắt buộc phải thiết lập riêng một phần **"Giả định dự báo & Cơ sở ước tính Lợi nhuận"** trước bảng Residual Income (RI).
- Giải trình rõ ràng mức độ tin cậy và cơ sở toán học/ logic của các giả định: Tăng trưởng tín dụng, xu hướng NIM tăng/giảm nhẹ, chi phí trích lập dự phòng (CoC) và tỷ lệ CIR dự báo cho năm nay và năm sau.
- Biểu đồ lịch sử PE/PB 32 quý phải được định dạng chiều ngang rộng **`12x5`** và thưa nhãn để tránh co cụm chồng chữ trên báo cáo.

---

## 23. BẢNG PEER BENCHMARK — CẬP NHẬT ĐỘNG (BẮT BUỘC)

### 23.1 Tổng quan

Mỗi khi chạy phân tích bất kỳ mã ngân hàng nào, hệ thống **BẮT BUỘC** cập nhật toàn bộ bảng so sánh 18 ngân hàng (`data/peer_benchmark.json`) với dữ liệu mới nhất **tại ngày phân tích đó**, vì P/B thay đổi hàng ngày theo thị giá.

### 23.2 Chỉ số cần cào cho mỗi NH

| Chỉ số | Mã tài khoản | Cách tính |
|---|---|---|
| NPL (%) | `nob42+nob43+nob44` / `bsb103` | Nhóm 3+4+5 / Tổng cho vay |
| NIM (%) | `isb27 * 4` / `bsb103` | NII quý × 4 / Cho vay (năm hóa) |
| CASA (%) | `nob66` / `nob65` | TG không kỳ hạn / Tổng TG KH |
| ROE (%) | `isa20 * 4` / `bsa78` | LNST quý × 4 / VCSH (năm hóa) |
| CIR (%) | `abs(isb39)` / `isb38` | OPEX / TOI |
| TTTD (%) | (`bsb103_q0 - bsb103_q1`) / `bsb103_q1` | Tăng trưởng tín dụng QoQ |
| Vốn hóa | `currentPrice × numberOfSharesMktCap / 1e9` | Từ company details API |
| **P/B** | `mcap / (bsa78 / 1e9)` | **TỰ TÍNH — CẤM dùng API** |

### 23.3 Tính P/B chuẩn xác (CẤM dùng API valuation)

**API `/valuation-history` hay bị block 403. KHÔNG sử dụng.**

```python
# ✅ ĐÚNG — Tự tính P/B từ dữ liệu gốc
# Bước 1: Vốn hóa (tỷ VND)
det = r_details.json().get("data", {})
price = det.get("currentPrice")           # VND thực tế: 61400.0 (không phải 61.4)
shares = det.get("numberOfSharesMktCap")  # Số CP lưu hành — key chính xác
mcap = (price * shares) / 1e9            # Tỷ VND

# Bước 2: VCSH từ Balance Sheet quý gần nhất
...
> **Kiểm tra thực tế Q1/2026**: VCB P/B ≈ 2.19x | BID ≈ 1.59x | VIB ≈ 1.12x

### 23.4 API Endpoints chuẩn cho Peer Benchmark

```
# Domain: iq.vietcap.com.vn (KHÔNG dùng api.vietcap.vn — bị block)
BASE = "https://iq.vietcap.com.vn/api/iq-insight-service/v1"

GET {BASE}/company/details?ticker={ticker}
    → currentPrice, numberOfSharesMktCap

GET {BASE}/company/{ticker}/financial-statement?section=BALANCE_SHEET&quarterly=true
    → bsa78 (VCSH), bsb103 (Cho vay KH)

GET {BASE}/company/{ticker}/financial-statement?section=INCOME_STATEMENT&quarterly=true
    → isa20 (LNST), isb27 (NII), isb38 (TOI), isb39 (OPEX)

GET {BASE}/company/{ticker}/financial-statement?section=NOTE&quarterly=true
    → nob42/43/44 (Nhóm 3/4/5), nob65 (Tổng TG), nob66 (CASA)
```

### 23.5 Cấu trúc file `data/peer_benchmark.json`

```json
{
  "_meta": { "updated": "2026-06-29", "source": "Vietcap Live API Engine" },
  "peers": [
    {
      "ticker": "VCB", "name": "Vietcombank",
      "npl": 0.63, "nim": 4.09, "casa": 32.55, "roe": 16.17,
      "cir": 32.5,  "pb": 2.19,  "cg": 4.78,   "mcap": 513038,
...
### 23.6 Hiển thị Ngày phân tích

Vì P/B thay đổi theo ngày, **bắt buộc ghi rõ ngày** ở 3 nơi:

**PDF** (footnote cuối bảng Peer Benchmark):
```python
today_str = datetime.datetime.now().strftime("%Y-%m-%d")
# → "Nguồn: BCTC Q1/2026 và Vietcap. P/B được cập nhật theo thị giá đóng cửa ngày: 2026-06-29."
```

**Web Dashboard** (`#peer-update-date` trong `index.html`):
```javascript
// app.js: renderPeerBenchmarkTable()
const updateDate = currentPeer?.date_updated || data._meta?.date;
dateEl.innerHTML = `Dữ liệu tính toán theo thị giá đóng cửa ngày: <b>${updateDate}</b>`;
```

**Bảng Web**: Mỗi hàng ngân hàng hiển thị ngày cập nhật nhỏ bên dưới tên.

### 23.7 Lỗi phổ biến khi triển khai

| Lỗi | Nguyên nhân | Giải pháp |
|---|---|---|
| `KeyError: 'NOTES'` | Key sai trong template_banking.py | Đổi thành `"NOTE"` (xem SECTIONS trong fetch_data.py) |
| `mcap: 6.14e-05` (gần 0) | Key sai: `outstandingShares` không tồn tại | Dùng `numberOfSharesMktCap` |
| P/B = 1.00x tất cả NH | API `/valuation-history` bị block 403 | Tự tính: `mcap / (bsa78/1e9)` |
| `name 'ThreadPoolExecutor' is not defined` | Import ở giữa file | Chuyển lên đầu: `from concurrent.futures import ThreadPoolExecutor` |
| `AttributeError: module 'datetime' has no attribute 'now'` | Dùng `datetime.now()` thay vì `datetime.datetime.now()` | Sửa thành `datetime.datetime.now()` |
