/* ════════════════════════════════════════════════════════
   AIC FA SYSTEM — app_retail.js (Bán Lẻ)
...
let chartStoreCount = null;
let chartEfficiency = null;
let chartQuarterlyGrowth = null;
let chartQuarterlyMarginRoe = null;

// ── Vietcap API base ──────────────────────────────────────
const VC_BASE = 'https://trading.vietcap.com.vn/api/iq-insight-service/v1';

// ═══════════════════════════════════════════════════════════
// SECTOR CONFIGURATION (retail)
// ═══════════════════════════════════════════════════════════
const SECTOR_CONFIG = {
    retail: {
        label: 'Bán Lẻ',
        ...
        income2Label: 'LN gộp',
        npatLabel: 'LNST cổ đông mẹ',
        showPE: true,
        primaryValuation: 'P/S+P/E theo mảng',
        extraMetrics: (ratios) => {
...
const SEGMENT_COLORS = { TGDD: '#3b82f6', DMX: '#f59e0b', BHX: '#10b981', Khac: '#a855f7' };

// ── Map sector string → config key ────────────────────────
function getSectorKey(sectorStr) {
    const s = (sectorStr || '').toLowerCase();
    if (s.includes('bán lẻ') || s.includes('ban le') || s.includes('retail') ||
        s.includes('consumer discretionary') || s.includes('personal') || s.includes('household')) return 'retail';
    return 'other';
}
...
// ═══════════════════════════════════════════════════════════
// LOAD STOCK DASHBOARD (retail-adaptive)
// ═══════════════════════════════════════════════════════════
async function loadStockDashboard(ticker) {
    document.documentElement.style.setProperty('--sector-color', '#8b5cf6');
    document.getElementById('view-overview').style.display = 'none';
    document.getElementById('view-analysis').style.display = 'flex';
    ...
    const pbData = ttmQuarters.map(r => r.pb && r.pb > 0 ? parseFloat(r.pb.toFixed(2)) : null);
    ...
    renderPEChart(quarterLabels, peData, cfg.color, peCarried);
    renderPBChart(quarterLabels, pbData, cfg.color);

    renderSegmentRevenueChart(localJson);
    renderStoreCountChart(localJson);
    renderEfficiencyChart(localJson);
    renderQuarterlyGrowthChart(localJson);
    renderQuarterlyMarginRoeChart(localJson);
    renderAssetQualityTable(localJson);
    renderSegmentStoreSnapshot(localJson);
...
function destroyCharts() {
    [chartRevNpat, chartEpsEquity, chartPE, chartPB, chartSegmentRevenue, chartStoreCount,
     chartEfficiency, chartQuarterlyGrowth, chartQuarterlyMarginRoe].forEach(c => { if (c) c.destroy(); });
    chartRevNpat = chartEpsEquity = chartPE = chartPB = chartSegmentRevenue = chartStoreCount =
    chartEfficiency = chartQuarterlyGrowth = chartQuarterlyMarginRoe = null;
}
...
// ═══════════════════════════════════════════════════════════
// CHART RENDERERS (retail-specific)
// ═══════════════════════════════════════════════════════════
function renderSegmentRevenueChart(localJson) {
    const card = document.getElementById('segment-revenue-card');
    const seg = localJson?.segments;
    if (!card || !seg || !seg.names || !seg.revenueForecast) { if (card) card.classList.add('hidden'); return; }
    card.classList.remove('hidden');

    const names = seg.names;
    const nYears = Math.max(...names.map(n => (seg.revenueForecast[n] || []).length));
    const forecastYears = localJson?.data?.years?.slice(-nYears) || Array.from({ length: nYears }, (_, i) => `N${i + 1}`);
    const labels = forecastYears.map(y => `${y}E`);

    const totalsByYear = labels.map((_, i) => names.reduce((s, n) => s + (seg.revenueForecast[n]?.[i] || 0), 0));
    ...
    chartSegmentRevenue = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: names.map(n => ({
                label: n,
                data: (seg.revenueForecast[n] || []).map(v => Math.round(v)),
        ...
        options: {
            ...
            scales: {
                ...
                y: { ...CHART_DEFAULTS.scales.y, stacked: true, title: { display: true, text: 'Tỷ VND', color: '#545f74', font: { size: 10 } } }
            },
            plugins: {
                ...CHART_DEFAULTS.plugins,
                tooltip: {
                    ...
                    callbacks: {
                        label: (c) => {
                            const total = totalsByYear[c.dataIndex] || 1;
                            const pct = (c.parsed.y / total * 100).toFixed(1);
                            return `${c.dataset.label}: ${formatNumber(c.parsed.y)} tỷ (${pct}%)`;
                        }
                    }
                }
            }
    ...
    const lastShares = names.map(n => ({ n, v: seg.revenueForecast[n]?.[lastIdx] || 0 }));
    const total = lastShares.reduce((s, x) => s + x.v, 0) || 1;
    const dominant = lastShares.sort((a, b) => b.v - a.v)[0];
    const analysisEl = document.getElementById('analysis-text-segment');
    if (analysisEl) {
        analysisEl.innerHTML = `
            Năm ${labels[lastIdx]}, mảng <strong>${dominant.n}</strong> chiếm tỷ trọng lớn nhất
            (${(dominant.v / total * 100).toFixed(1)}% doanh thu dự phóng).<br/>
            ${lastShares.map(x => `${x.n}: ${(x.v / total * 100).toFixed(1)}%`).join(' · ')}
        `;
    }
}

function renderStoreCountChart(localJson) {
    const card = document.getElementById('store-count-card');
    const seg = localJson?.segments;
    const monthly = seg?.monthly;
    if (!card || !seg || !monthly || !monthly.labels?.length) { if (card) card.classList.add('hidden'); return; }
    card.classList.remove('hidden');

    const histLabels = monthly.labels;
    const fcYears = localJson?.data?.years?.slice(-3) || [];
    const fcLabels = fcYears.map(y => `${y}E`);
    const labels = [...histLabels, ...fcLabels];

    const primarySegs = ['TGDD', 'DMX'];
    const secondarySeg = 'BHX';

    const datasets = [];
    primarySegs.forEach(n => {
        const hist = monthly.storeCount[n] || [];
        const fc = seg.storeCountForecast?.[n] || [];
        datasets.push({
            label: n, data: [...hist, ...fc], borderColor: SEGMENT_COLORS[n], backgroundColor: SEGMENT_COLORS[n] + '18',
            borderWidth: 2, pointRadius: 0, tension: 0.2, yAxisID: 'y', spanGaps: true
        });
    });
    {
        const hist = monthly.storeCount[secondarySeg] || [];
        const fc = seg.storeCountForecast?.[secondarySeg] || [];
        datasets.push({
            label: secondarySeg, data: [...hist, ...fc], borderColor: SEGMENT_COLORS[secondarySeg],
            backgroundColor: SEGMENT_COLORS[secondarySeg] + '18', borderWidth: 2.5, pointRadius: 0, tension: 0.2,
            yAxisID: 'y1', spanGaps: true
        });
    }

    const ctx = document.getElementById('storeCountChart').getContext('2d');
    if (chartStoreCount) chartStoreCount.destroy();
    const step = Math.max(1, Math.floor(labels.length / 18));
    chartStoreCount = new Chart(ctx, {
        type: 'line',
        data: { labels, datasets },
        options: {
            ...CHART_DEFAULTS,
            scales: {
                x: { ...CHART_DEFAULTS.scales.x, ticks: { ...CHART_DEFAULTS.scales.x.ticks, callback: (v, i) => i % step === 0 ? labels[i] : '' } },
                y: { ...CHART_DEFAULTS.scales.y, position: 'left', title: { display: true, text: 'TGDĐ / ĐMX (CH)', color: SEGMENT_COLORS.TGDD, font: { size: 10 } } },
                y1: { ...CHART_DEFAULTS.scales.y, position: 'right', grid: { drawOnChartArea: false }, title: { display: true, text: 'BHX (CH)', color: SEGMENT_COLORS.BHX, font: { size: 10 } } }
            }
        }
    });

    const nowCount = seg.storeCountNow || {};
    const analysisEl = document.getElementById('analysis-text-store');
    if (analysisEl) {
        analysisEl.innerHTML = `
            Hiện tại: TGDĐ ${formatNumber(nowCount.TGDD)} CH, ĐMX ${formatNumber(nowCount.DMX)} CH (đã bão hòa),
            BHX ${formatNumber(nowCount.BHX)} CH (đang tăng tốc mở rộng).<br/>
            Dự phóng BHX đạt ${formatNumber(seg.storeCountForecast?.BHX?.[seg.storeCountForecast.BHX.length - 1])} CH vào cuối giai đoạn dự báo.
        `;
    }
}

function renderEfficiencyChart(localJson) {
    ...
    const cutoffCount = 60; // 5 năm gần nhất (theo tháng)
    const start = Math.max(0, monthly.labels.length - cutoffCount);
    const labels = monthly.labels.slice(start);
    const names = seg.names.filter(n => n !== 'Khac');
    ...
    chartEfficiency = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: names.map(n => ({
                label: n,
                data: (monthly.efficiency[n] || []).slice(start),
                borderColor: SEGMENT_COLORS[n] || '#64748b',
                backgroundColor: (SEGMENT_COLORS[n] || '#64748b') + '14',
        ...
        options: {
            ...
            scales: {
                x: { ...CHART_DEFAULTS.scales.x, ticks: { ...CHART_DEFAULTS.scales.x.ticks, callback: (v, i) => i % step === 0 ? labels[i] : '' } },
                y: { ...CHART_DEFAULTS.scales.y, title: { display: true, text: 'Tỷ VND/CH/tháng', color: '#545f74', font: { size: 10 } } }
            }
        }
    });

    const eff = seg.efficiencyNow || {};
    const analysisEl = document.getElementById('analysis-text-efficiency');
    if (analysisEl) {
        analysisEl.innerHTML = `
            Hiệu quả hiện tại (TB 3 tháng gần nhất): TGDĐ ${formatNumber(eff.TGDD, 2)} tỷ/CH/tháng,
            ĐMX ${formatNumber(eff.DMX, 2)}, BHX ${formatNumber(eff.BHX, 2)}.<br/>
            <span style="font-size:0.8rem;color:var(--text-dim)">Hiệu quả đi ngang/tăng chậm cho thấy mảng đã bão hòa; hiệu quả tăng nhanh cho thấy còn dư địa tăng trưởng (chỉ báo SSSG tương đương).</span>
        `;
    }
}

function renderQuarterlyGrowthChart(localJson) {
    const card = document.getElementById('quarterly-growth-card');
    const qr = localJson?.quarterlyRatios;
    if (!card || !qr || !qr.labels?.length) { if (card) card.classList.add('hidden'); return; }
    card.classList.remove('hidden');

    const labels = qr.labels;
    const growthPct = qr.ni_yoy.map(v => (v !== null && v !== undefined && v >= -0.3 && v <= 0.7) ? v * 100 : null);

    const ctx = document.getElementById('quarterlyGrowthChart').getContext('2d');
    if (chartQuarterlyGrowth) chartQuarterlyGrowth.destroy();
    const step = Math.max(1, Math.floor(labels.length / 16));
    chartQuarterlyGrowth = new Chart(ctx, {
        type: 'bar',
        data: {
            labels,
            datasets: [
                { label: 'LNST theo quý (tỷ VND)', data: qr.ni, backgroundColor: '#2980b9cc', borderRadius: 3, yAxisID: 'y' },
                { label: 'Tăng trưởng LNST YoY (%, -30%~70%)', data: growthPct, type: 'line', borderColor: '#e74c3c', backgroundColor: '#e74c3c18', borderWidth: 2, pointRadius: 3, tension: 0.25, spanGaps: false, yAxisID: 'y1' }
            ]
        },
        options: {
            ...CHART_DEFAULTS,
            scales: {
                x: { ...CHART_DEFAULTS.scales.x, ticks: { ...CHART_DEFAULTS.scales.x.ticks, callback: (v, i) => i % step === 0 ? labels[i] : '' } },
                y: { ...CHART_DEFAULTS.scales.y, position: 'left', title: { display: true, text: 'LNST (tỷ VND)', color: '#2980b9', font: { size: 10 } } },
                y1: { ...CHART_DEFAULTS.scales.y, position: 'right', grid: { drawOnChartArea: false }, title: { display: true, text: 'Tăng trưởng YoY (%)', color: '#e74c3c', font: { size: 10 } } }
            }
        }
    });

    const analysisEl = document.getElementById('analysis-text-quarterly-growth');
    if (analysisEl) {
        const lastNi = qr.ni[qr.ni.length - 1];
        const lastYoy = qr.ni_yoy[qr.ni_yoy.length - 1];
        analysisEl.innerHTML = `
            LNST quý gần nhất (${labels[labels.length - 1]}): <strong>${formatNumber(lastNi, 1)} tỷ VND</strong>
            ${lastYoy !== null && lastYoy !== undefined ? `, tăng trưởng YoY ${(lastYoy * 100).toFixed(1)}%` : ''}.<br/>
            <span style="font-size:0.8rem;color:var(--text-dim)">Các quý tăng trưởng >70% hoặc <-30% được ẩn khỏi đường tăng trưởng vì nền so sánh cùng kỳ quá thấp/quá cao (lỗ/gần hòa vốn hoặc nền cực lớn), gây méo biểu đồ.</span>
        `;
    }
}

function renderQuarterlyMarginRoeChart(localJson) {
    const card = document.getElementById('quarterly-margin-roe-card');
    const qr = localJson?.quarterlyRatios;
    if (!card || !qr || !qr.labels?.length) { if (card) card.classList.add('hidden'); return; }
    card.classList.remove('hidden');

    const labels = qr.labels;
    const ctx = document.getElementById('quarterlyMarginRoeChart').getContext('2d');
    if (chartQuarterlyMarginRoe) chartQuarterlyMarginRoe.destroy();
    const step = Math.max(1, Math.floor(labels.length / 16));
    chartQuarterlyMarginRoe = new Chart(ctx, {
        type: 'line',
        data: {
            labels,
            datasets: [
                { label: 'Biên LNG (%)', data: qr.gpm.map(v => v !== null ? v * 100 : null), borderColor: '#2980b9', borderWidth: 2, pointRadius: 0, tension: 0.25, yAxisID: 'y' },
                { label: 'Biên LNST (%)', data: qr.npm.map(v => v !== null ? v * 100 : null), borderColor: '#e74c3c', borderWidth: 2, pointRadius: 0, tension: 0.25, yAxisID: 'y' },
                { label: 'ROE annualized (%)', data: qr.roe.map(v => v !== null && v !== undefined ? v * 100 : null), borderColor: '#f59e0b', borderWidth: 2, borderDash: [4, 3], pointRadius: 0, tension: 0.25, yAxisID: 'y1' }
            ]
        },
        options: {
            ...CHART_DEFAULTS,
            scales: {
                x: { ...CHART_DEFAULTS.scales.x, ticks: { ...CHART_DEFAULTS.scales.x.ticks, callback: (v, i) => i % step === 0 ? labels[i] : '' } },
                y: { ...CHART_DEFAULTS.scales.y, position: 'left', title: { display: true, text: 'Biên LN (%)', color: '#545f74', font: { size: 10 } } },
                y1: { ...CHART_DEFAULTS.scales.y, position: 'right', grid: { drawOnChartArea: false }, title: { display: true, text: 'ROE (%)', color: '#f59e0b', font: { size: 10 } } }
            }
        }
    });

    const analysisEl = document.getElementById('analysis-text-quarterly-margin');
    if (analysisEl) {
        const last = { gpm: qr.gpm[qr.gpm.length - 1], npm: qr.npm[qr.npm.length - 1], roe: qr.roe[qr.roe.length - 1] };
        analysisEl.innerHTML = `
            Quý gần nhất (${labels[labels.length - 1]}): Biên LNG <strong>${last.gpm !== null ? (last.gpm * 100).toFixed(1) + '%' : '-'}</strong>,
            Biên LNST <strong>${last.npm !== null ? (last.npm * 100).toFixed(1) + '%' : '-'}</strong>,
            ROE (annualized) <strong>${last.roe !== null && last.roe !== undefined ? (last.roe * 100).toFixed(1) + '%' : '-'}</strong>.
        `;
    }
}

function renderAssetQualityTable(localJson) {
    const card = document.getElementById('asset-quality-card');
    const flags = localJson?.assetQuality;
    if (!card) return;
    if (!flags || !flags.length) { card.classList.add('hidden'); return; }
    card.classList.remove('hidden');
    const tbody = card.querySelector('tbody');
    const flagClass = (f) => f === 'Cảnh báo' ? 'down' : (f === 'Theo dõi' ? 'flat' : 'up');
    tbody.innerHTML = flags.map(f => `
        <tr>
            <td>${f.check}</td>
            <td>${f.value}</td>
            <td><span class="badge-growth ${flagClass(f.flag)}">${f.flag}</span></td>
        </tr>
    `).join('');
}

function renderSegmentStoreSnapshot(localJson) {
    const card = document.getElementById('segment-store-card');
    const seg = localJson?.segments;
    if (!card) return;
    if (!seg || !seg.storeCountNow) { card.classList.add('hidden'); return; }
    card.classList.remove('hidden');
    const grid = document.getElementById('segment-store-grid');
    const now = seg.storeCountNow;
    grid.innerHTML = Object.entries(now).map(([n, v]) => `
        <div class="bank-kpi-item">
            <div class="bank-kpi-label">${n}</div>
            <div class="bank-kpi-value" style="color:${SEGMENT_COLORS[n] || '#8b5cf6'}">${formatNumber(v)}</div>
            <div style="font-size:0.7rem;color:var(--text-dim)">cửa hàng</div>
        </div>
    `).join('');
}
...
function renderQuarterlyAndYTDEvaluation(ticker, liveData, localJson, cfg) {
    ...
    if (yoyNpat !== null && yoyNpat > 20 && (yoyRev === null || yoyRev < 5 || (npatRevGap !== null && npatRevGap > 40))) {
        ...
        commentary += ` <br/><span style="color:#f59e0b">⚠️ Lưu ý: LNST tăng mạnh (${formatGrowth(yoyNpat)}) ${gapText} — dấu hiệu cho thấy khả năng có thu nhập đột biến, không lặp lại, chứ không hoàn toàn đến từ tăng trưởng hoạt động kinh doanh cốt lõi. Nên đối chiếu thuyết minh BCTC quý ${latestQ.quarter}/${latestQ.yearReport} để xác nhận nguồn gốc lợi nhuận.</span>`;
    }
...
function renderFinancialSnapshotTable(localJson, sectorKey) {
    ...
    const grossMargin = ratios.gpm || ratios.gross_margin || [];
    ...
    const rows = [
        ...
        { label: 'Biên LNST (%)', arr: netMargin, fmt: fmtPct },
...
function renderValuationSnapshot(localJson, livePrice) {
    ...
    const target = val.weightedTarget ?? val.base;
    const upside = val.upsidePct ?? (target ? ((target / currPrice - 1) * 100) : (val.upside ?? 0));
    const recText  = val.recommendation ?? val.recommend ?? (upside >= 15 ? 'MUA' : upside < -5 ? 'BAN' : 'THEO DÕI');
    ...
    if (el('snap-coe')) el('snap-coe').textContent = (val.coe != null ? val.coe.toFixed(2) : (val.COE != null ? val.COE.toFixed(2) : '-')) + '%';
    // Retail: methods = {pe, pb, ri, ps_pe}
    if (el('snap-pe-median')) el('snap-pe-median').textContent = val.methods?.ps_pe ? fmt(val.methods.ps_pe) : '-';
    if (el('snap-pb-median')) el('snap-pb-median').textContent = val.methods?.pb ? fmt(val.methods.pb) : '-';
    if (el('snap-pb-attractive')) el('snap-pb-attractive').textContent = val.methods?.ri ? fmt(val.methods.ri) : '-';
    if (el('snap-bear')) el('snap-bear').textContent = val.bear ? fmt(val.bear) : '-';
    if (el('snap-pb-target')) el('snap-pb-target').textContent = val.bull ? fmt(val.bull) : '-';
}
...
function renderThesisAndRisks(thesisList, risksList) {
    ...
    const defaultRisks = [
        'Rủi ro kinh tế vĩ mô và sức mua tiêu dùng suy yếu.',
        'Cạnh tranh trong ngành bán lẻ ngày càng gay gắt.',
        'Rủi ro thực thi chiến lược mở rộng chuỗi cửa hàng mới.',
    ];
