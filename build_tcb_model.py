#!/usr/bin/env python3
"""
TCB (Techcombank) — Excel Model + PDF Report Generator
...
EXCHANGE = "HOSE"
INDUSTRY = "Ngân hàng (Bank quy mô lớn)"
...
PRICE, SHARES, MARKET_CAP = fetch_current_market_data(TICKER, 33400, 7086240414)
...
# Margin assumptions
nim_fc  = [3.40, 3.45, 3.50]  # % (annualized, can cu: Q1/2026 actual annualized = 3.44%, 2025 binh quan = 3.78%)
...
# Industry averages (simple mean of peer banks excluding TCB)
INDUSTRY_AVG = {}
for metric in ["NPL","NIM","CASA","ROE","CIR","P_B","CREDIT_GROWTH"]:
    vals = [v for k,v in PEER_DATA[metric].items() if k != "TCB" and v > 0]
...
def create_pdf():
    ...
    story.append(Paragraph(f"{TICKER} ({COMPANY}) là ngân hàng TMCP quy mô lớn với vốn hóa ~{MARKET_CAP/1e9:,.0f} tỷ. "
                          f"TCB đang giao dịch tại P/B {pb_current:.2f}x và P/E {MARKET_CAP/(np_hist[-1]*1e9):.1f}x — "
    ...
    story.append(Paragraph(f"NPL của {TICKER} đang ở mức {npl_ratio_hist[-1]}% — thấp hơn trung bình ngành ({INDUSTRY_AVG['NPL']:.2f}%). "
                          ...
                          f"CAR của TCB thuộc nhóm cao trong ngành, tạo dư địa tăng trưởng tín dụng.", body_style))
    story.append(Spacer(1, 3*mm))
    ...
    conclusion = (
        ...
        f"TCB có CASA cao ({casa_ratio_hist[-1]}%), NPL thấp ({npl_ratio_hist[-1]}%) và NIM tốt ({nim_hist[-1]}%) — "
