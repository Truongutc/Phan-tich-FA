#!/usr/bin/env python3
"""
VIB (VIB) — Excel Model + PDF Report Generator
...
OUT_DIR = r"E:\1. Projects\4. AIC - FA\Bao cao\VIB"
MONTH = "2026-06"
EXCEL_FILE = os.path.join(OUT_DIR, f"VIB_Model_{MONTH}.xlsx")
PDF_FILE   = os.path.join(OUT_DIR, f"VIB_Phan_Tich_{MONTH}.pdf")
...
EXCHANGE = "HOSE"
INDUSTRY = "Ngân hàng (Bank quy mô vừa)"
...
PRICE, SHARES, MARKET_CAP = fetch_current_market_data(TICKER, 16000, 3404005710)
...
# ── Quarterly NPAT for PE carry logic (LYST am → PE = previous quarter) ──
...
# Median per year for each year in data
...
# Margin assumptions
nim_fc  = [4.40, 4.45, 4.50]  # % (annualized, can cu: VIB NIM historical ~4.5-5%, du phong bao toan 4.40-4.50%)
...
# ══════════════════════════════════════════════════════
# RESIDUAL INCOME MODEL — Công thức chuẩn tài chính
# ══════════════════════════════════════════════════════
# RI(t) = EPS(t) - BVPS(t-1) × COE
#       = BVPS(t-1) × (ROE(t) - COE)
# BVPS(t) = BVPS(t-1) + EPS(t)   [giả định không chia cổ tức]
# Giá trị RI = BVPS(0) + PV[RI(1..3)] + PV[Continuing Value]
# Continuing Value = RI(3) × (1+g) / (COE - g)
# Nếu RI < 0 → ROE < COE → công ty chưa tạo giá trị vượt chi phí vốn → bình thường

ri_results = []
bv = bvps_base  # BVPS đầu kỳ = BVPS cuối 2025
for i in range(3):
    bv_start = bv
    eps_i = eps_fc_calc[i]            # EPS năm i (VND/share)
    capital_charge = bv_start * COE   # Chi phí vốn = BVPS_đầu × COE
    ri = eps_i - capital_charge       # RI = EPS - Chi phí vốn
    ri_results.append(ri)
    bv = bv_start + eps_i             # BVPS tăng bằng EPS (không chia cổ tức)

# Debug print để kiểm tra
print(f"  RI Detail:")
for i, (ri, eps) in enumerate(zip(ri_results, eps_fc_calc)):
    bvps_beg = bvps_base if i == 0 else bvps_base + sum(eps_fc_calc[:i])
    roe_i = eps / bvps_beg * 100
    print(f"    {years_fc[i]}F: EPS={eps:,.0f} | BVPS_beg={bvps_beg:,.0f} | ROE={roe_i:.1f}% | COE={COE*100:.0f}% | RI={ri:,.0f}")

# Continuing Value: dùng RI năm cuối cùng để ngoại suy vĩnh viễn
# Nếu RI_terminal âm thì CV âm → phản ánh đúng thực tế
cv = ri_results[-1] * (1 + terminal_growth) / (COE - terminal_growth) if (COE - terminal_growth) > 0 else 0

# PV của RI và Continuing Value
...
print(f"  PV(RI 3Y)={pv_ri:,.0f} VND | PV(CV)={pv_cv:,.0f} VND | RI Value={ri_value:,.0f} VND")

# P/B MULTIPLE — 3 MỨC PHÂN PHỐI LỊCH SỬ
# ══════════════════════════════════════════════════════
...
bvps_forward = bvps_base + eps_fc_calc[0]   # = bvps_2025 + eps_2026F
pb_value = pb_target * bvps_forward        # Định giá dùng P/B mục tiêu (upper half median)

print(f"  P/B: median={pb_all_median:.2f}x | current={pb_current:.2f}x | target={pb_target:.2f}x")
print(f"  BVPS_forward={bvps_forward:,.0f} | P/B Value={pb_value:,.0f} VND")
...
PEER_BANKS = ["VIB","ACB","VCB","BID","CTG","MBB","VPB","HDB","VIB","LPB","STB","SHB","EIB","MSB","OCB","NAB","BAB","VAB"]
PEER_DATA = {
    "NPL":   {"VIB": 1.53, "ACB": 1.07, "VCB": 1.14, "BID": 1.52, "CTG": 1.34, "MBB": 1.43, "VPB": 2.81, "HDB": 1.64, "VIB": 2.44, "LPB": 1.46, "STB": 2.18, "SHB": 2.01, "EIB": 2.65, "MSB": 1.74, "OCB": 1.89, "NAB": 1.52, "BAB": 1.38, "VAB": 1.29},
    "NIM":   {"VIB": 4.52, "ACB": 5.49, "VCB": 3.21, "BID": 2.85, "CTG": 2.92, "MBB": 4.15, "VPB": 5.12, "HDB": 4.38, "VIB": 4.67, "LPB": 3.84, "STB": 3.41, "SHB": 3.12, "EIB": 2.96, "MSB": 3.55, "OCB": 3.28, "NAB": 2.71, "BAB": 2.58, "VAB": 2.69},
    "CASA":  {"VIB": 36.8, "ACB": 24.7, "VCB": 32.1, "BID": 18.5, "CTG": 16.2, "MBB": 38.4, "VPB": 12.4, "HDB": 15.8, "VIB": 8.9, "LPB": 19.2, "STB": 14.3, "SHB": 11.6, "EIB": 13.1, "MSB": 16.5, "OCB": 9.8, "NAB": 5.12, "BAB": 4.89, "VAB": 4.77},
    "ROE":   {"VIB": 19.6, "ACB": 21.6, "VCB": 20.8, "BID": 16.2, "CTG": 15.8, "MBB": 22.4, "VPB": 17.2, "HDB": 20.1, "VIB": 18.5, "LPB": 16.8, "STB": 12.4, "SHB": 13.2, "EIB": 10.5, "MSB": 14.8, "OCB": 11.6, "NAB": 13.8, "BAB": 12.1, "VAB": 14.4},
    "CIR":   {"VIB": 30.2, "ACB": 34.1, "VCB": 31.5, "BID": 37.8, "CTG": 36.2, "MBB": 32.5, "VPB": 42.1, "HDB": 38.6, "VIB": 35.4, "LPB": 34.8, "STB": 41.2, "SHB": 39.5, "EIB": 44.1, "MSB": 37.2, "OCB": 40.3, "NAB": 35.6, "BAB": 34.2, "VAB": 33.0},
    "P_B":   {"VIB": 1.35, "ACB": 1.72, "VCB": 2.85, "BID": 1.68, "CTG": 1.42, "MBB": 1.58, "VPB": 1.18, "HDB": 1.65, "VIB": 1.22, "LPB": 1.38, "STB": 0.94, "SHB": 0.88, "EIB": 0.76, "MSB": 0.92, "OCB": 0.82, "NAB": 0.78, "BAB": 0.72, "VAB": 0.86},
    "CREDIT_GROWTH": {"VIB": 14.1, "ACB": 15.6, "VCB": 9.5, "BID": 12.4, "CTG": 11.8, "MBB": 13.2, "VPB": 8.6, "HDB": 16.2, "VIB": 11.5, "LPB": 18.4, "STB": 7.2, "SHB": 9.6, "EIB": 6.8, "MSB": 10.2, "OCB": 8.4, "NAB": 12.6, "BAB": 9.1, "VAB": 10.8},
    "MCAP":  {"VIB": 108.6, "ACB": 125.4, "VCB": 485.2, "BID": 214.6, "CTG": 156.8, "MBB": 124.5, "VPB": 95.2, "HDB": 52.8, "VIB": 35.6, "LPB": 42.1, "STB": 38.4, "SHB": 22.6, "EIB": 18.9, "MSB": 16.2, "OCB": 12.4, "NAB": 8.6, "BAB": 5.4, "VAB": 9.1},
}
def peer_val(metric):
    return {b: PEER_DATA[metric].get(b, 0) for b in PEER_BANKS}

# Industry averages (simple mean of peer banks excluding VIB)
INDUSTRY_AVG = {}
for metric in ["NPL","NIM","CASA","ROE","CIR","P_B","CREDIT_GROWTH"]:
    vals = [v for k,v in PEER_DATA[metric].items() if k != "VIB" and v > 0]
...
def create_excel():
    ...
    assumptions = [
        ...
        ("P/B mục tiêu (x)", [pb_median_year.get(y, 0) for y in years_hist] + [round(pb_target,2), round(pb_target,2), round(pb_target,2)]),
        ("P/E median (x)", [pe_median_year.get(y, 0) for y in years_hist] + [round(pe_all_median,1)]*3),
    ]
    ...
    thesis = [
        ("Chất lượng TS", f"NPL {npl_ratio_hist[-1]}% (tốt). Nợ nhóm 2 ~{gr2_ratio_hist[-1]}%. LLR {prov_hist[-1]/max(npl_total_hist[-1],1)*100:.0f}% (ổn).", "Tốt"),
        ("Tăng trưởng", f"Credit growth ~{PEER_DATA['CREDIT_GROWTH'].get(TICKER,0)}%. CASA cao ({casa_ratio_hist[-1]}%) hỗ trợ NIM.", "Tốt"),
        ("Lợi nhuận", f"NIM {nim_hist[-1]}% (tốt). CASA {casa_ratio_hist[-1]}% (cao). Phí dịch vụ tăng.", "Tốt"),
        ("Hiệu quả", f"ROE {roe_hist[-1]}%. CIR {cir_hist[-1]}% (hiệu quả cao).", "Tốt"),
        ("Định giá", f"P/B {pb_current:.2f}x. P/E {MARKET_CAP/(np_hist[-1]*1e9):.1f}x. Target {weighted_target:,.0f} VND (+{upside:.0f}%).", "Trung bình"),
        ("Vĩ mô & Ngành", "LS điều hành ổn định. NHNN room 14%. Cạnh tranh NIM gay gắt.", "Trung bình"),
    ]
    ...
    ws.column_dimensions['A'].width = 30; [ws.column_dimensions[get_column_letter(j)].__setattr__('width', 16) for j in range(2, len(all_years) + 2)]
    ...
    sum_items = [
        ...
        ("P/B (x)", [pb_median_year.get(y, 0) for y in years_hist] + [pb_target]*3),
        ("P/E (x)", [pe_median_year.get(y, 0) for y in years_hist] + [round(MARKET_CAP/(np_fc[i]*1e9),2) for i in range(3)]),
    ]
    # Add CAGR row
    ...
    ws.cell(row=cagr_row, column=1, value=f"CAGR {years_hist[0]}-{years_hist[-1]} (%)").font = FMT_BOLD
    cagr_items = [
    ...
    for i, (label, vals) in enumerate(cagr_items):
        r = cagr_row + 1 + i
        cagr_val = (vals[-1] / vals[0]) ** (1 / (len(vals) - 1)) - 1 if (len(vals) > 1 and vals[0]) else 0
    ...
    # ── Sheet 13: PE/PB History (historical only) ──
    ...
    # Chart 2: PE & PB History Line Chart
    chart2 = LineChart()
    chart2.title = "Lịch sử P/E & P/B (Vietcap)"
    chart2.style = 13
    chart2.width = 20
    chart2.height = 12
    
    ws_pe_pb = wb["13_PE_PB_History"]
    max_r = len(pe_periods) + 2
    
    # Thêm data cho PE (Cột B) và PB (Cột C)
    data_pe = Reference(ws_pe_pb, min_col=2, max_col=3, min_row=2, max_row=max_r)
    cats_pe = Reference(ws_pe_pb, min_col=1, min_row=3, max_row=max_r)
    chart2.add_data(data_pe, titles_from_data=True)
    chart2.set_categories(cats_pe)
    
    ws2.add_chart(chart2, "A25")
...
def register_vn_fonts():
    ...
    fonts_to_try = [("arial.ttf", "Arial"), ("arialbd.ttf", "Arial-Bold"),
                    ...
                    ("CALIBRI.TTF", "Calibri"), ("CALIBRIB.TTF", "Calibri-Bold"),
                    ("segoeui.ttf", "Segoe UI"), ("segoeuib.ttf", "Segoe UI-Bold"),
...
def create_charts():
    ...
    # Font detection with Vietnamese support
    import matplotlib.font_manager as fm
    _avail = {f.name for f in fm.fontManager.ttflist}
    _preferred = ['Arial', 'Calibri', 'Segoe UI', 'Tahoma', 'DejaVu Sans', 'sans-serif']
    _chosen = next((f for f in _preferred if f in _avail), 'sans-serif')
    plt.rcParams['font.family'] = 'sans-serif'
    plt.rcParams['font.sans-serif'] = [_chosen] if _chosen != 'sans-serif' else ['DejaVu Sans', 'Arial', 'sans-serif']
...
def create_pdf():
    ...
    story.append(Paragraph(f"{TICKER} ({COMPANY}) là ngân hàng TMCP quy mô lớn với vốn hóa ~{MARKET_CAP/1e9:,.0f} tỷ. "
                          f"VIB đang giao dịch tại P/B {pb_current:.2f}x và P/E {MARKET_CAP/(np_hist[-1]*1e9):.1f}x — "
                          f"mức định giá hợp lý so với trung bình ngành. CASA {casa_ratio_hist[-1]}% (cao) "
                          f"và NIM {nim_hist[-1]}% là thế mạnh cốt lõi.", body_style))
    ...
    story.append(Paragraph(f"• CASA cao ({casa_ratio_hist[-1]}%) — nguồn vốn rẻ giúp NIM duy trì ở mức {nim_hist[-1]}%", bullet_style))
    story.append(Paragraph(f"• NPL chỉ {npl_ratio_hist[-1]}% — chất lượng tài sản tốt, dưới TB ngành", bullet_style))
    story.append(Paragraph(f"• ROE ~19%+ — hiệu quả sinh lời cao và ổn định qua các chu kỳ", bullet_style))
    story.append(Spacer(1, 3*mm))
    story.append(Paragraph("<b>3 Rủi ro:</b>", h2_style))
    story.append(Paragraph("• Cạnh tranh NIM từ bank lớn (VCB, MBB, ACB) gây áp lực thu hẹp spread", bullet_style))
    story.append(Paragraph("• Danh mục TPDN (trái phiếu doanh nghiệp) cần theo dõi chất lượng", bullet_style))
    story.append(Paragraph(f"• P/B {pb_current:.2f}x không còn rẻ — cần tăng trưởng LNST để duy trì định giá", bullet_style))
    story.append(PageBreak())
    
    # ── Asset Quality ──
    story.append(Paragraph("2. Chất lượng Tài sản (Asset Quality)", h1_style))
    story.append(Paragraph(f"NPL của {TICKER} đang ở mức {npl_ratio_hist[-1]}% — thấp hơn trung bình ngành ({INDUSTRY_AVG['NPL']:.2f}%). "
                          f"Nợ nhóm 2 (Gr2) ở mức {gr2_ratio_hist[-1]:.2f}%. "
                          f"LLR/NPL khoảng {prov_hist[-1]/max(npl_total_hist[-1],1)*100:.0f}% — ở mức ổn định. "
                          f"CAR của VIB thuộc nhóm cao trong ngành, tạo dư địa tăng trưởng tín dụng.", body_style))
    story.append(Spacer(1, 3*mm))
    ...
    if pe_all_vals:
        ...
        story.append(Paragraph(f"• P/E hien tai: {pe_latest:.1f}x (trung vi all-time: {pe_all_median:.1f}x) "
                              f"— {'cao hon' if pe_latest > pe_all_median else 'thap hon'} trung vi", bullet_style))
        story.append(Paragraph(f"• P/B hien tai: {pb_latest:.2f}x (trung vi all-time: {pb_all_median:.2f}x) "
                              f"— {'cao hon' if pb_latest > pb_all_median else 'thap hon'} trung vi", bullet_style))
        story.append(Paragraph(f"• Dinh gia hien tai o vung {'cao' if pe_latest > pe_all_median * 1.1 else 'thap' if pe_latest < pe_all_median * 0.9 else 'trung binh'} lich su", body_style))
    story.append(Spacer(1, 3*mm))
    ...
    swot_data = [
        ...
        [f"CASA cao ({casa_ratio_hist[-1]}%)\nNPL thấp ({npl_ratio_hist[-1]}%)\nCIR thấp ({cir_hist[-1]}%)\nNIM tốt ({nim_hist[-1]}%)",
         f"TPDN rủi ro tiềm ẩn\nP/B không rẻ ({pb_current:.2f}x)\nPhụ thuộc vào thị trường BĐS"],
        ["OPPORTUNITIES", "THREATS"],
        ["Số hóa → tăng CASA\nRoom tín dụng mở rộng\nTăng phí dịch vụ (bancassurance)",
         "Cạnh tranh NIM từ bank lớn\nBiến động LS huy động\nSiết TPDN từ NHNN"],
    ]
    ...
    conclusion = (
        ...
        f"{TICKER} đang giao dịch tại P/B {pb_current:.2f}x, P/E {MARKET_CAP/(np_hist[-1]*1e9):.1f}x. "
        f"Định giá theo Residual Income + P/B cho target {weighted_target:,.0f} VND, upside {ups:+.1f}%. "
        f"VIB có CASA cao ({casa_ratio_hist[-1]}%), NPL thấp ({npl_ratio_hist[-1]}%) và NIM tốt ({nim_hist[-1]}%) — "
        f"đây là những thế mạnh cốt lõi so với mặt bằng ngân hàng Việt Nam. "
        f"Rủi ro chính đến từ danh mục TPDN và áp lực cạnh tranh từ các bank lớn khác. "
        f"Khuyến nghị: {'MUA với kỳ vọng CASA tiếp tục cải thiện và NIM duy trì' if ups > 15 else 'THEO DÕI, chờ tín hiệu rõ ràng hơn về tăng trưởng tín dụng.' if ups > 5 else 'NẮM GIỮ với tầm nhìn dài hạn.' if ups > -5 else 'BÁN do upside hạn chế.'}"
    )
...
if __name__ == "__main__":
    ...
    print(f"Price: {PRICE:,} VND | MCap: {MARKET_CAP/1e9:,.0f} ty")
