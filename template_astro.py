...
def build_forecast_timeline(upcoming_natal_aspects, upcoming_eclipses, retro_stations, daily_pressure=None, days_ahead=FORECAST_DAYS_AHEAD):
    """Hợp nhất góc chiếu TRANSIT CHẠM LÁ SỐ VN-INDEX sắp tới + nhật/nguyệt thực + ngày hành tinh
    đổi chiều (station) trong `days_ahead` ngày tới thành 1 dòng thời gian duy nhất, mỗi sự kiện
    kèm diễn giải rule-based. Trả list sắp theo ngày tăng dần.

    upcoming_natal_aspects: list {date, transit, natal, aspect} từ
    fetch_astro_data.find_upcoming_transit_to_natal_exact_aspects() — thay cho góc chiếu
    transit-transit chung chung trước đây (đã bỏ hoàn toàn, xem VNINDEX_NATAL_DATETIME/kiến trúc
    lá số VN-Index — user 2026-08-10: "bỏ hoàn toàn cách tính cũ, giữ lại chiêm tinh tài chính theo
    cách mới", vì cách cũ không mang đặc thù riêng của VN-Index).
    ...
    sự kiện màu xanh (tích cực khi đứng riêng lẻ) xuất hiện đúng lúc chỉ số TỔNG lại đang giảm — chỉ
    giải thích bằng chữ (cảnh báo phía trên bảng) là chưa đủ, cần gắn SỐ THẬT vào từng dòng để không
    phải đối chiếu ngược lên biểu đồ."""
    ...
    for a in upcoming_natal_aspects:
        ...
        events.append({
            ...
            "title": f"{a['transit']} {a['aspect']} {a['natal']} (lá số VN-Index)",
            "interpretation": (f"Hành tinh transit chạm đúng vị trí {a['natal']} trong lá số VN-Index — liên quan "
                                f"\"{PLANET_MEANING.get(a['transit'], '')}\" tác động trực tiếp lên chủ đề "
...
def build_astro_data():
    ...
    # Lá số VN-Index (natal chart) + transit-to-natal — DUY NHẤT phương pháp tính điểm áp
    # lực/thuận lợi + dự báo góc chiếu trong toàn trang (user 2026-08-10: "bỏ hoàn toàn cách tính
    # cũ [transit-transit chung, không đặc thù VN-Index], giữ lại chiêm tinh tài chính theo cách
    # mới [transit-to-natal riêng VN-Index], tôi thấy cách mới đúng hơn" — thay hẳn, không còn giữ
    # song song 2 chỉ số như bản trước). only_hard=False (kế thừa lý do sửa lỗi user 2026-08-04):
    # chỉ lấy góc chiếu cứng khiến bảng dự báo toàn màu căng thẳng, cần cả góc chiếu mềm để cân
    # bằng bức tranh thực tế.
    ...
    out = {
        ...
        "forecastTimeline": build_forecast_timeline(upcoming_natal_aspects, upcoming_eclipses, retro_stations, daily_pressure=daily_pressure),
        "backtest": build_backtest_data(natal_positions_raw, days_back=3650),
    }
